let leanto = (mapsquare) => {
    let state = {
        ...
        possibleoutputs: function() {
            return []; // aka this block does not return anything
        },
        update: function() {  // Handles updating this block every tick
            ...
            if(state.status==0) {
                ...
                state.counter++;
                state.endurance+=5;
                workpoints = workpoints -1;
                $("#"+ state.tile.id +"progress").css({"width": state.counter*0.5});
                if(state.counter>=120) { // aka 2 minutes
                    state.status = 1;
        ...
        drawpanel: function() {
            $("#sidepanel").html('<b>Lean-To</b><br />'+
                ...
                '<br />');
            if(state.status==0) {
                $("#sidepanel").append('Status: <span id="sidepanelstatus">Building: '+ (Math.floor(state.counter/1.2)) +'% complete</span>');
            }else{
                $("#sidepanel").append('Status: <span id="sidepanelstatus">In use. '+ (Math.floor(state.endurance/6)) +'% lifespan remaining</span>');
            }
        },
        updatepanel: function() {
            if(state.status==0) {
                ...
                $("#sidepanelstatus").html('In use. '+ (Math.floor(state.counter*100.0/state.endurance)) +'% lifespan remaining');
            }
        },
