...
export const blockShowsOutputItems = state => ({
    ...
    displayItemsOnHand() {
        ...
        return itemsList
            .map(
                ele =>
                    `<span style="margin-left:30px">${ele.name}: <b>${ele.count}</b></span><br />`
            )
            .join("");
    }
});
...
export const blockHasWorkerPriority = state => ({
    ...
    showPriority() {
        ...
        if (state.priority > 111) {
            $("#sidepanel").append(
                '<img src="img/arrowleft3.png" id="sidepanelprioritydown3" /> '
            );
            document
                .getElementById("sidepanelprioritydown3")
                .addEventListener("click", () =>
                    game.blockSelect.setPriority(-100)
                );
        }
        if (state.priority > 11) {
            $("#sidepanel").append(
                '<img src="img/arrowleft2.png" id="sidepanelprioritydown2" /> '
            );
            document
                .getElementById("sidepanelprioritydown2")
                .addEventListener("click", () =>
                    game.blockSelect.setPriority(-10)
                );
        }
        $("#sidepanel").append(`
        ...
        if (top > 11 && state.priority < top - 10) {
            $("#sidepanel").append(
                '<img src="img/arrowright2.png" id="sidepanelpriorityup2" /> '
            );
            document
        ...
        if (top > 111 && state.priority < top - 100) {
            $("#sidepanel").append(
                '<img src="img/arrowright3.png" id="sidepanelpriorityup3" /> '
            );
            document
...
export const blockHandlesFood = state => ({
    ...
    deleteWithFood() {
        ...
        for (let i = 0; i < state.onhand.length; i++) {
            let pos = game.foodList.findIndex(
                ele => state.onhand[i].id === ele.id
            );
            if (pos === -1) {
                console.log(
                    `Failed to find ${state.onhand[i].name} (id=${state.onhand[i].id}) in foodList`
                );
                continue;
            }
...
export const blockDeletesClean = state => ({
    ...
    showDeleteLink() {
        ...
        $("#sidepanel").append(
            '<a href="#" id="sidepaneldelete">Delete Block</a>'
        );
        document
            .getElementById("sidepaneldelete")
            .addEventListener("click", () => game.blockSelect.deleteBlock());
    },
