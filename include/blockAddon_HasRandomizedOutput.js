...
export const blockHasRandomizedOutput = state => ({
    ...
    processCraft(efficiency) {
        ...
        if (state.outputItems[0].name === undefined) {
            console.log(
                `Error: block type of ${state.name} has not updated to most recent version of blockHasRandomizedOutput (the outputItems structure needs additional content`
            );
        }
        ...
        if (state.counter >= state.craftTime) {
            ...
            if (selected.isFood === true) {
                state.onhand.push(
                    food(selected.name, selected.shelfLife, state)
                );
            } else {
        ...
        $("#" + state.tile.id + "progress").css({
            width: (state.counter / state.craftTime) * 60
        });
    },
